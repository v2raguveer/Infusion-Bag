...
cbag <- SCTransform(bag.284.F, vars.to.regress = "percent.mt", verbose = FALSE)
cbag <- RunPCA(cbag, verbose=FALSE)
cbag1 <- RunTSNE(cbag, reduction = "pca", dims = 1:30, verbose = FALSE)
cbag <- RunUMAP(cbag,reduction="pca", dims=1:30, verbose=FALSE)
cbag <- FindNeighbors(cbag, dims=1:30, verbose = FALSE)
cbag <- FindClusters(cbag, verbose=FALSE)
cbag1 <- FindNeighbors(cbag1, dims=1:30, verbose = FALSE)
cbag1 <- FindClusters(cbag1, verbose=FALSE)
DimPlot(cbag, label = TRUE) + NoLegend()
DimPlot(cbag1, label = TRUE) + NoLegend()
...
top10 <- head(VariableFeatures(cbag), 10)
plota <- VariableFeaturePlot(cbag)
plota1 <- LabelPoints(plota, points=top10, repel=TRUE)
LabelPoints(plota, points=top10, repel=TRUE)
top10 <- head(VariableFeatures(cbag1), 10)
plotb <- VariableFeaturePlot(cbag1)
plotb1 <- LabelPoints(plotb, points=top10, repel=TRUE)
LabelPoints(plotb, points=top10, repel=TRUE)
VlnPlot(object = cbag, features = c("CD8A"), pt.size = 0.2, ncol = 1)
VlnPlot(object = cbag, features = c("IL7R"), pt.size = 0.2, ncol = 1)
FeaturePlot(object = cbag, features = c("CD8A", "GZMK", "CCL5"), pt.size = 0.2, ncol = 3)
FeaturePlot(object = cbag1, features = c("CD8A", "GZMK", "CCL5"), pt.size = 0.2, ncol = 3)
DimHeatmap(cbag)
DimHeatmap(cbag1)
cbag.markers <- FindAllMarkers(cbag, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
cbag.markers %>% group_by(cluster) %>% top_n(n = 2, wt = avg_logFC)
top10 <- cbag.markers %>% group_by(cluster) %>% top_n(n = 10, wt = avg_logFC)
top50 <- top10[1:50, ]
DoHeatmap(cbag, features = top50$gene) + NoLegend()
